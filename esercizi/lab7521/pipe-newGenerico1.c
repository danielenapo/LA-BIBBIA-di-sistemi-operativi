...
int main (int argc, char** argv){
    ...
    if (pid == 0){/* figlio */
        ...
        i=0;
        while (read(fd, &msg, 1)){/*  il  padre  ha  concordato  con  il  figlio  che  gli  mandera'  solo  stringhe  e  quindi dobbiamo sostituire il terminatore di linea con il terminatore di stringa */
	    if(msg == '\n'){
	    	mess[i] = '\0';
	    	i++;
    ...
    j=0;
    /*while (read ( piped[0], inpbuf, MSGSIZE)){  dato che il figlio gli ha inviato delle stringhe, il padre le puo' scrivere direttamente con una printf 
        printf ("%d: %s\n", j, inpbuf);
        j++;
    }*/
    while(read(piped[0], &i, sizeof(i))){
    	read(piped[0], inpbuf, i);
    	printf ("%d: %s\n", j, inpbuf);
        j++;
    }
    printf("Padre %d letto %d messaggi dalla pipe\n", getpid(), j);/* padre aspetta il figlio */
